...
export class VodFoundation extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps) {
    ...
    this.templateOptions.description =
      "(SO0146) v1.2.0: Video on Demand on AWS Foundation Solution Implementation";
    /**
      ...
      cfn_nag: {
        rules_to_suppress: [
          ...
          },
          {
            id: "W51",
            reason:
              "Logs bucket is private and does not require a bucket policy",
          },
        ],
      },
    };
        ...
        rules_to_suppress: [
          {
            id: "W51",
            reason:
              "source bucket is private and does not require a bucket policy",
          },
        ],
      },
    };
    ...
    const customResourceLambda = new lambda.Function(this, "CustomResource", {
      ...
      initialPolicy: [
        ...
        new iam.PolicyStatement({
          ...
          resources: [
            `arn:aws:mediaconvert:${cdk.Aws.REGION}:${cdk.Aws.ACCOUNT_ID}:*`,
          ],
        }),
      ],
    });
    /** get the cfn resource for the role and attach cfn_nag rule */
    const cfnCustomResource = customResourceLambda.node.findChild(
      "Resource"
      ...
      cfn_nag: {
        rules_to_suppress: [
          ...
          },
          {
            id: "W92",
            reason:
              "Invalid warning: lambda does not need ReservedConcurrentExecutions",
          },
        ],
      },
    };
    ...
    /** get the cfn resource for the role and attach cfn_nag rule */
    const cfnJobSubmit = jobSubmit.node.findChild(
      "Resource"
    ) as lambda.CfnFunction;
    ...
    const jobComplete = new lambda.Function(this, "JobComplete", {
      ...
      retryAttempts: 0,
      description:
        "Triggered by Cloudwatch Events,processes completed MediaConvert jobs.",
      environment: {
        MEDIACONVERT_ENDPOINT: customResourceEndpoint.getAttString("Endpoint"),
        CLOUDFRONT_DOMAIN:
    ...
    const cfnJobComplete = jobComplete.node.findChild(
      "Resource"
    ) as lambda.CfnFunction;
    ...
    cdk.Tags.of(appRegistry).add(
      "appRegistryApplicationName",
      "vod-foundation-stack"
    );
    ...
    new cdk.CfnOutput(this, "SourceBucket", {
      ...
      description:
        "Source S3 Bucket used to host source video and MediaConvert job settings files",
