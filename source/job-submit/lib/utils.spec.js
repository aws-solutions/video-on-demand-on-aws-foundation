...
describe("Utils GetJobSettings", () => {
  ...
  it("GetJobSettings Success test", async () => {
    ...
    await utils.getJobSettings().catch((err) => {
      expect(err.Error.toString()).toEqual(
        "Error: Invalid settings file in s3"
      );
    });
  });
...
describe("Utils UpdateJobSettings", () => {
  ...
  it("UpdateJobSettings Failed test", async () => {
    await utils
      .updateJobSettings(inValidSettings, "inputPath", "outputPath", {}, "role")
      .catch((err) => {
        expect(err.toString()).toEqual(
          "Error: 'OutputGroupSettings.Type is not a valid type. Please check/test your job settings file.'"
        );
      });
  });
});
